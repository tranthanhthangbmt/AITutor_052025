...
for idx, msg in enumerate(st.session_state.messages[1:]):  
    ...
    # ✅ Phát âm thanh cho Gia sư AI nếu bật
    if role == "🤖 Gia sư AI" and st.session_state.get("enable_audio_playback", True):
        if is_last:
            # 👉 Luôn phát message cuối của AI
            render_audio_block(msg["parts"][0]["text"], autoplay=True)
        elif st.session_state.get("read_lesson_first", False) and previous_msg:
            # 👉 Phát ghép với message trước đó nếu là phần giữa và bật đọc bài học
            combined_text = previous_msg["parts"][0]["text"] + "\n\n" + msg["parts"][0]["text"]
            render_audio_block(combined_text, autoplay=True)
